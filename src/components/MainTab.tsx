...
const MainTab = ({ onStreamerChange }: MainTabProps) => {
  ...
  const handleAccountsChange = (val: string) => {
    ...
    // НЕ очищаем textarea, только после успешной загрузки из файла
  };
  ...
  const handleFileProxies = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setProxies(valid);
      setProxiesText(""); // Очищаем textarea только после успешной загрузки файла
    };
    reader.readAsText(file);
    e.target.value = "";
  };
  ...
  return (
    ...
    {/* hidden file inputs */}
    <input type="file" accept=".txt" ref={fileInputAccRef} className="hidden" onChange={handleFileAccounts} />
    <input type="file" accept=".txt" ref={fileInputProxyRef} className="hidden" onChange={handleFileProxies} />
  </ScrollArea>
          <CardContent>
            <Textarea
              placeholder="Вставьте аккаунты здесь..."
